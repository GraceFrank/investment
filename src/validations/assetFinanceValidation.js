...
export const assetFinanceSchema = Joi.object({
  ...
  vendor_street_address: Joi.string().required().min(1).max(255).trim(),
  vendor_street_address2: Joi.string().min(1).max(255).trim(),
  vendor_city: Joi.string().required().min(1).max(255).trim(),
  vendor_state: Joi.string().required().min(1).max(255).trim(),
  vendor_country: Joi.string().required().min(1).max(255).trim(),
});
